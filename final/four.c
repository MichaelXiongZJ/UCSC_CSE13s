...
// void add_word(WordCountNode *node, WordCountTable *table) {
//   size_t which_bucket = hash(word) % table->num_buckets;
//   table->buckets[which_bucket] = node;
// }
...
WordCountTable *merge_word_count_tables(WordCountTable *table_a,
  ...
  for(size_t i = 0; i < (table_a->num_buckets); i++){
    if(table_a->buckets[i]){
      WordCountNode *temp = table_a->buckets[i];
      char* word = temp->word;
  ...
  for(size_t i = 0; i < (table_b->num_buckets); i++){
    if(table_b->buckets[i]){
      WordCountNode *temp = table_b->buckets[i];
      char* word = temp->word;
      int count = temp->count;

      if(get_word_count(word, out) != 0){
        set_word_count(word, count + get_word_count(word, out), out);
      }else{
        set_word_count(word, count, out);
      }
    }
